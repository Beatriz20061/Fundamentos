...
if page == "🏠 Página Inicial":
    ...
    científicos presentes nos cinco projetos académicos desenvolvidos no ISCTE Sintra[cite: 435, 437].
    ...
    * **Módulo 1 :** Axiomas de Grupos, Isometrias lineares, Caleidoscópios, a arte de Escher e as cápsides de vírus icosaédricos[cite: 606, 607, 608].
    * **Módulo 2 :** Estudo e classificação de todas as pavimentações periódicas através dos 17 Grupos Cristalográficos do Plano[cite: 695, 696].
    * **Módulo 3 :** Modelação proposicional, Teoria de Grafos e Combinatória de paridade aplicadas ao jogo *Number Match*[cite: 469].
    * **Módulo 4 :** Investigação da distribuição de primos na reta, Espiral de Ulam e Função Totiente de Euler[cite: 51, 52].
    * **Módulo 5 :** Números Triangulares, Números Perfeitos, as propriedades cíclicas do Número 9 e o Último Teorema de Fermat[cite: 317, 318, 319, 320].
    """)
...
# ==============================================================================
    ...
    st.markdown('<div class="academic-header"><h1>🔵 Módulo 1: Grupos de simetria</h1><p>Autoras: Catarina Pereira, Beatriz Correia | Março 2026</p></div>', unsafe_allow_html=True)
    ...
    with tab1:
        with st.expander("1.1 Isometrias e Estrutura Abstrata da Teoria de Grupos", expanded=True):
            ...
            * **Reflexões Deslizantes (*glide reflections*):** Combinação de uma reflexão com uma translação paralela ao eixo de reflexão[cite: 620].
            ...
            Formalmente, um grupo $(G, *)$ satisfaz quatro propriedades essenciais: **Fecho** ($a*b \in G$) [cite: 626], **Associatividade** ($(a*b)*c = a*(b*c)$) [cite: 627], **Elemento Neutro** ($e*a=a$) [cite: 628] e **Elemento Inverso** ($a*a^{-1}=e$)[cite: 629]. Um exemplo clássico é o grupo das rotações do triângulo equilátero, que forma o grupo cíclico $C_3$[cite: 631].
            """)
            
        with st.expander("1.2 Eficiência Viral e Simetrias na Natureza e na Arte", expanded=True):
            st.markdown("""
            **Explicação Teórica Exaustiva:**
            Muitos vírus ditos "esféricos" apresentam, na verdade, uma estrutura baseada no **icosaedro**, um dos sólidos platónicos mais simétricos[cite: 635]. A geometria icosaédrica oferece eixos de rotação múltiplos de ordem 2, 3 e 5[cite: 636, 639]. Isto permite que subunidades proteicas idênticas (capsómeros) se encaixem perfeitamente para formar uma cápsula (capsídeo) fechada e estável[cite: 639]. Esta organização repetitiva confere uma enorme **eficiência genética**, permitindo que o vírus utilize um único gene para codificar uma pequena proteína que se organiza através de rotações para formar uma estrutura grande e complexa[cite: 640, 641]. O adenovírus, o poliovírus ou o vírus Zika seguem o modelo estrutural de Caspar-Klug[cite: 642, 643].
            
            Na estrutura tridimensional de proteínas e do ADN observamos também a simetria helicoidal, que utiliza uma combinação de rotação e translação, assemelhando-se às reflexões deslizantes aplicadas a um eixo central[cite: 645, 647, 648]. Na arte, M. C. Escher explorou intensamente estas isometrias para pavimentar o plano de forma regular sem deixar espaços vazios[cite: 654, 655, 661]. No caleidoscópio, o efeito visual resulta de múltiplas reflexões entre espelhos dispostos em triângulo, gerando simetrias rotacionais de ângulo $360/n$ e repetições cíclicas infinitas[cite: 663, 664, 665, 666, 667, 670].
            """)
...
# ==============================================================================
    ...
    st.markdown('<div class="academic-header"><h1>🟤 Módulo 2: Os 17 Grupos Cristalográficos do Plano</h1><p>Autoras: Catarina Pereira, Beatriz Correia | Março 2026</p></div>', unsafe_allow_html=True)
    ...
    with tab1:
        ...
        with st.expander("2.2 Análise Comparativa Profunda dos Subgrupos p4, p6 e pm", expanded=True):
            ...
            * **Grupo p6 (Simetria Hexagonal):** É um dos mais simétricos e apresenta rotações de ordem 6 (ângulos de 60°, 120°, 180°, 240° e 300°), ordem 3 e ordem 2[cite: 746, 747, 751, 752]. As suas translações organizam-se numa rede hexagonal, muito comum em estruturas naturais como favos de mel pela sua eficiência geométrica[cite: 748, 756]. **Não possui reflexões**, o que o distingue de *p6m*[cite: 754, 757].
            * **Grupo pm (Simetria Axial):** É um dos exemplos mais simples entre os grupos com reflexão[cite: 762]. Caracteriza-se pela presença de **eixos de reflexão paralelos**, combinados com translações perpendiculares a esses eixos, gerando bandas simétricas especulares em rede retangular[cite: 763]. Não possui rotações[cite: 768].
            """)
...
# ==============================================================================
    ...
    st.markdown('<div class="academic-header"><h1>🟢 Módulo 3: A Lógica Matemática por Trás do Jogo Number Match</h1><p>Autoras: Catarina Pereira, Beatriz Correia | Abril 2026</p></div>', unsafe_allow_html=True)
    ...
    with tab1:
        ...
        with st.expander("3.2 Modelação em Teoria de Grafos e Estratégia Combinatória de Paridade", expanded=True):
            st.markdown("""
            **Explicação Teórica Exaustiva:**
            O tabuleiro do jogo pode ser representado como um grafo dinâmico, no qual os números correspondem a vértices e as relações de jogada válida a arestas[cite: 467]. Nesta perspetiva, o jogo pode ser entendido como um processo iterativo de remoção de vértices e arestas, cuja estrutura se altera ao longo do tempo[cite: 468]. À medida que o jogador elimina pares, o grafo sofre transformações sucessivas[cite: 510]. Neste contexto, o grau de um vértice — isto é, o número de arestas incidentes — assume um papel importante[cite: 512]. Vértices de grau elevado correspondem a números com várias jogadas possíveis, enquanto vértices de grau zero representam números isolados que não podem ser eliminados[cite: 513]. Estes vértices isolados contribuem diretamente para estados de bloqueio do tabuleiro[cite: 514].
            
            **Análise Combinatória e o Impacto do Número 5:**
            Técnicas combinatórias baseiam-se em princípios de contagem, análise de frequências e argumentos de paridade[cite: 519]. Cada número entre 1 e 9 possui exatamente um único parceiro que permite obter a soma 10 ($1\\leftrightarrow9, 2\\leftrightarrow8, 3\\leftrightarrow7, 4\\leftrightarrow6$)[cite: 530, 531, 532, 533, 534]. O número 5 constitui um caso particular, pois **apenas pode formar par consigo mesmo** ($5\\leftrightarrow5$)[cite: 535, 537]. Isto implica que a sua frequência no tabuleiro deve ser impreterivelmente **par** para permitir a eliminação completa e evitar a criação de números isolados[cite: 537]. Se aparecer um número ímpar de vezes, pelo menos um 5 ficará inevitavelmente isolado, por um argumento direto de paridade[cite: 577]. Por este motivo, a presença de 5 representa um fator crítico para a estabilidade do jogo[cite: 538].
            """)
...
# ==============================================================================
    ...
    st.markdown('<div class="academic-header"><h1>🟣 Módulo 4: Padrões dos Primos : Da Espiral à Totiente</h1><p>Autoras: Catarina Pereira, Beatriz Correia | Março 2026</p></div>', unsafe_allow_html=True)
    ...
    with tab1:
        with st.expander("4.1 A Natureza e Distribuição Irregular dos Números Primos", expanded=True):
            st.markdown("""
            **Explicação Teórica Exaustiva:**
            A Teoria dos Números é um dos ramos mais antigos e fascinantes da matemática[cite: 45]. Apesar de lidar com objetos aparentemente simples, os números inteiros, revela estruturas profundas e padrões inesperados[cite: 46]. Esta ideia é destacada por G. H. Hardy na sua obra *A Mathematician's Apology* (1940), onde defende que a matemática pura possui uma beleza intrínseca, comparável à arte, precisamente porque revela ordem onde antes se supunha irregularidade[cite: 47]. Hardy, um dos maiores teóricos dos números do século XX, via nesta área um exemplo perfeito de como estruturas elegantes podem emergir de objetos aparentemente elementares[cite: 48]. A distribuição dos números primos, irregular à primeira vista mas repleta de padrões subtis, é um dos casos mais emblemáticos dessa visão[cite: 49].
        ...
        with st.expander("4.3 A Função Totiente de Euler e Estrutura Multiplicativa Modular", expanded=True):
            ...
            **Fórmula Multiplicativa Geral:**
            Todo o número natural admite uma decomposição em fatores primos[cite: 212]. Se $n=p_1^{a_1}p_2^{a_2}\dots p_k^{a_k}$ em que $p_1,p_2,\dots,p_k$ são os fatores primos distintos de $n$, então[cite: 213, 214]:
            """)
            st.latex(r"\varphi(n)=n\prod_{i=1}^{k}\left(1-\frac{1}{p_i}\right)")
            st.markdown("""
            Além disso, a função totiente cumpre a propriedade multiplicativa quando os fatores são coprimos entre si: $\\varphi(ab)=\\varphi(a)\\varphi(b)$ se $mdc(a,b)=1$[cite: 218, 219].
            
            **Propriedades Fundamentais:**
            1. **Totiente de um Número Primo:** Se $p$ é primo, então $\\varphi(p)=p-1$, porque todos os inteiros menores que um primo são coprimos com ele[cite: 225, 226, 227, 228].
            2. **Totiente de uma Potência de Primo:** Para qualquer primo $p$ e inteiro $k \ge 1$: $\\varphi(p^k)=p^k-p^{k-1}=p^k(1-\\frac{1}{p})$ [cite: 230, 231], pois num número da forma $p^k$, apenas os múltiplos de $p$ não são coprimos, e existem $p^{k-1}$ desses múltiplos[cite: 233, 234].
    ...
    with tab2:
        ...
        r4_2 = st.radio("2. Quanto vale a função totiente de Euler para n=12, sabendo que os coprimos até ele são {1, 5, 7, 11}?", 
                        ["12", "6", "4", "2"], key="r4_2")
...
# ==============================================================================
    ...
    st.markdown('<div class="academic-header"><h1>🟡 Módulo 5: Padrões Numéricos e Estruturas Aritméticas</h1><p>Autoras: Catarina Pereira, Beatriz Correia | Maio 2026</p></div>', unsafe_allow_html=True)
    ...
    with tab1:
        with st.expander("5.1 Números Triangulares e a Abordagem Geometrica de Contagem", expanded=True):
            st.markdown("""
            **Explicação Teórica:**
            Os números triangulares pertencem ao grupo dos chamados números figurados, que se caracterizam por serem números que podem ser representados através de arranjos geométricos de pontos regulares[cite: 324]. Esta abordagem visual é de extrema importância na aprendizagem matemática, pois transforma problemas de contagem abstrata numa perceção geométrica muito intuitiva[cite: 325]. Eles representam a quantidade de pontos necessária para construir um triângulo equilátero com um determinado número de linhas[cite: 326]. Como cada nova linha adiciona sempre mais um ponto do que a linha anterior, um número triangular corresponde diretamente à soma dos primeiros números naturais[cite: 327].
            """)
            st.latex(r"T_{n}=1+2+3+\cdot\cdot\cdot+n=\frac{n(n+1)}{2}")
            st.markdown("""
            Esta fórmula matemática pode ser explicada através de um raciocínio geométrico muito simples que é atribuído historicamente a Gauss[cite: 330]. Se escrevermos a sequência da soma na sua ordem direta e, logo abaixo, na sua ordem inversa, conseguimos agrupar os termos verticalmente aos pares[cite: 331]. Ao somarmos as duas equações, cada par de termos resulta invariavelmente no valor de $n+1$[cite: 332]. Dado que existem exatamente $n$ termos nesta sequência, a soma total das duas linhas passa a ser escrita como $2T_{n}=n(n+1)$[cite: 333]. Logo, basta dividir o resultado por dois para isolar o termo inicial[cite: 334].
            
            **Aplicação Concreta no Quotidiano:**
            Os números triangulares possuem diversas aplicações práticas em problemas reais de contagem e organização[cite: 349]. Um exemplo clássico é a modelação e contagem de ligações em redes de comunicação[cite: 350]. Se tivermos uma sala com 6 pessoas e quisermos saber quantos apertos de mão únicos ocorrem se todas se cumprimentarem uma única vez, o cálculo resolve-se através do número triangular $T_{5}$[cite: 351]:
            """)
            st.latex(r"T_5 = \frac{5 \times (5+1)}{2} = 15 \text{ apertos de mão}")
            st.markdown("Este mesmo raciocínio aplica-se ao determinar o número de canais diretos necessários para interligar computadores ou servidores numa rede sem que haja repetições de caminhos[cite: 355].")
            
        with st.expander("5.2 Números Perfeitos e Harmonia de Divisores", expanded=True):
            st.markdown("""
            **Explicação Teórica Exaustiva:**
            Um número perfeito define-se como um número inteiro positivo que é exatamente igual à soma de todos os seus divisores próprios, considerando-se divisores próprios todos os fatores que são estritamente menores do que o próprio número[cite: 357]. Esta definição clássica remonta aos *Elementos de Euclides*, refletindo o interesse dos antigos matemáticos gregos por números que apresentassem propriedades estéticas de equilíbrio e harmonia aritmética[cite: 358].
            
            **Exemplos Clássicos:**
            * **Número 6:** Os seus divisores próprios são {1, 2, 3}[cite: 360]. Ao somarmos estes fatores, obtemos: $1+2+3=6$[cite: 361, 362].
            * **Número 28:** Os seus divisores próprios são {1, 2, 4, 7, 14}[cite: 363]. A sua soma combinada resulta em: $1+2+4+7+14=28$[cite: 363, 364].
            
            A análise profunda dos números perfeitos pares revela uma ligação direta com uma classe especial de números primos conhecidos como Primos de Mersenne, que assumem a forma matemática de $2^{p}-1$, onde o expoente $p$ é também um número primo[cite: 366]. Euclides demonstrou formalmente que, sempre que o termo $2^{p}-1$ for de facto um número primo, é possível gerar um número perfeito par através da fórmula[cite: 367]:
            """)
            st.latex(r"n=2^{p-1}(2^{p}-1)")
            st.markdown("""
            Seguindo outra linguagem formal recorrendo à função soma dos divisores $\\sigma(n)$ [cite: 373], que atribui a cada inteiro positivo a soma de todos os seus divisores integrais incluindo o próprio número [cite: 373], diz-se que um número é perfeito sempre que satisfizer a identidade $\\sigma(n)=2n$[cite: 374]. No caso prático do número 6, os seus divisores totais são 1, 2, 3 e 6, cuja soma total resulta em $\\sigma(6)=12$, que equivale precisamente ao dobro do número original ($2\times6$)[cite: 374]. Todos os perfeitos pares são também números triangulares[cite: 376]. Permanecem em aberto mistérios relativos à existência de termos perfeitos ímpares[cite: 377].
            """)
            
        with st.expander("5.3 Exploração do Número 9 na Base Decimal", expanded=True):
            st.markdown("""
            **Explicação Teórica:**
            O número 9 ocupa um lugar de destaque no âmbito da aritmética elementar devido à organização do nosso sistema de numeração posicional em base 10[cite: 380]. Pelo facto de o número 9 ser precisamente uma unidade inferior à base do sistema ($10-1$), cria-se um conjunto único de simetrias repetitivas, regras de divisibilidade facilitadas e padrões visuais muito claros nos seus algarismos[cite: 382]. 
            
            Qualquer número inteiro positivo é sempre congruente com a soma dos seus próprios algarismos em módulo 9: $n \equiv \\text{soma dos dígitos de } n \\pmod 9$[cite: 383, 384]. Esta propriedade justifica o critério de divisibilidade por 9[cite: 385]. Adicionalmente, os resultados da tabuada elementar do 9 funcionam como um verdadeiro espelho numérico que ilustra com precisão a complementaridade entre as dezenas e as unidades[cite: 391]. Os resultados de $9\times2=18$ e $9\times9=81$ são o espelho um do outro, o mesmo acontecendo com o par $9\times3=27$ e $9\times8=72$[cite: 394, 395]. Em todos estes produtos, a soma dos dígitos mantém-se fixa no valor 9 ($1+8=9$ ou $2+7=9$)[cite: 395].
            
            Esta estabilidade estende-se também ao comportamento das potências do número 9, cuja explicação algébrica assenta na relação matemática onde $9^{n}=(10-1)^{n}$[cite: 396]. Ao expandirmos esta expressão através do Binómio de Newton, todos os termos desenvolvidos na equação passam a ser múltiplos diretos de 10, com a única exceção do último termo[cite: 397]. Analisando a redução digital destas potências, as somas dos algarismos geram ciclos regulares invariáveis (ex: $9^1=9$, $9^2=81 \\rightarrow 9$, $9^3=729 \\rightarrow 18 \\rightarrow 9$)[cite: 400].
            """)
            
        with st.expander("5.4 O Último Teorema de Fermat e a Limitação de Padrões", expanded=True):
            st.markdown("""
            **Explicação Teórica:**
            O Último Teorema de Fermat constitui o encerramento ideal para o estudo de estruturas aritméticas, pois ilustra como pequenas alterações numa expressão matemática podem fazer com que um padrão de infinitas soluções desapareça por completo[cite: 402]. No caso do expoente $n=2$, a equação transforma-se na clássica expressão do Teorema de Pitágoras: $a^2+b^2=c^2$[cite: 403, 404]. Esta estrutura admite uma infinidade de soluções inteiras e positivas, conhecidas como triplas pitagóricas, como a tripla 3-4-5 ($3^2+4^2=5^2$)[cite: 405, 406, 407].
            
            Contudo, no século XVII, Pierre de Fermat introduziu uma rutura neste cenário[cite: 412]. Fermat afirmou na margem do seu livro que a equação geral:
            """)
